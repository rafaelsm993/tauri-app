...
export const AnilistAPI = {
  searchAnime: (query: string, page = 1, genre?: string) =>
    invoke<RawAnilistPage>("anilist_search_anime", { query, page, genre: genre ?? null }).then(
      (r) => mapPage(r, "anime"),
    ),

  searchManga: (query: string, page = 1, genre?: string) =>
    invoke<RawAnilistPage>("anilist_search_manga", { query, page, genre: genre ?? null }).then(
      (r) => mapPage(r, "manga"),
    ),
