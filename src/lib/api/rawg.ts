...
function mapDetail(raw: any): MediaDetail {
  ...
  return {
    ...
    developer: Array.isArray(raw.developers)
      ? raw.developers.map((d: any) => d.name).join(", ")
      : "",
    publisher: Array.isArray(raw.publishers)
      ? raw.publishers.map((p: any) => p.name).join(", ")
      : "",
...
export const RawgAPI = {
  ...
  genres: (): Promise<GenreOption[]> => invoke<RawRawgGenres>("rawg_genres").then(mapRawgGenres),
};
