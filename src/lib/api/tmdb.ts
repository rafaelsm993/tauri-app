...
export const TmdbAPI = {
  discoverMovies: (page = 1, genre?: number) =>
    invoke<RawPage>("tmdb_discover_movies", { page, genre: genre ?? null }).then((r) =>
      mapPage(r, mapMovie),
    ),
  ...
  discoverTv: (page = 1, genre?: number) =>
    invoke<RawPage>("tmdb_discover_tv", { page, genre: genre ?? null }).then((r) =>
      mapPage(r, mapTv),
    ),
  ...
  tvGenres: (): Promise<GenreOption[]> => invoke<RawGenreList>("tmdb_genres_tv").then(mapGenres),
};
