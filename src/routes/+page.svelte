<script lang="ts">
  ...
  function handleCardClick(item: MediaItem) {
    ...
    const id = item.id;
    goto(resolve("/media/[type]/[id]", { type, id: encodeURIComponent(String(id)) }));
  }
...
<div class="page">
  <header class="page-header">
    <SearchBar {onSearch} placeholder="Buscar filmes, séries, anime, mangá, livros…" />
    ...
    <main class="main-col">
      <p class="page-context">
        ...
        {:else if activeGenre !== null}
          ...
          <button class="link-btn" onclick={() => switchGenre(null)}> ← Todos os gêneros </button>
        {:else}
          Explorar por gênero
        {/if}
      </p>
