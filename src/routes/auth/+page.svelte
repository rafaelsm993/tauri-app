<script lang="ts">
  ...
  let existingUsers = $state<{ id: string; username: string }[]>([]);
  let profilesLoading = $state(true);

  $effect(() => {
    invoke<{ id: string; username: string }[]>('list_users')
      .then(users => { existingUsers = users; })
      .catch(() => { existingUsers = []; })
  ...
  async function handleRegister() {
    ...
    try {
      ...
      // Redirect to login tab with username pre-filled — no auto-login
        ...
        {#if !profilesLoading && existingUsers.length > 0}
          ...
          <div class="profile-list">
            {#each existingUsers as u (u.id)}
              <button
                class="profile-chip"
                class:active={loginUsername === u.username}
                onclick={() => selectProfile(u.username)}
              >
                <span class="profile-avatar">{u.username[0].toUpperCase()}</span>
                <span class="profile-name">{u.username}</span>
              </button>
            {/each}
          </div>
        {/if}
...
<style lang="scss">
  ...
  .profile-chip {
    ...
    font-size: 0.8rem;
    cursor: pointer;
    transition: border-color $dur-fast, background $dur-fast, color $dur-fast;
